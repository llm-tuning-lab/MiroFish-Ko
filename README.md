...
| 모듈 | 위치 | 설명 |
|------|------|------|
| **simulation_config_generator** | `backend/app/services/simulation_config_generator.py` | 시드 기반 시뮬레이션 설정 생성 |
| **ontology_generator** | `backend/app/services/ontology_generator.py` | 엔터티 관계 추출 (GraphRAG) |
| **report_agent** | `backend/app/services/report_agent.py` | 시뮬레이션 후 리포트 생성 |
| **zep_graph_memory_updater** | `backend/app/services/zep_graph_memory_updater.py` | 동적 시계열 메모리 업데이트 |
| **llm_client** | `backend/app/utils/llm_client.py` | OpenAI SDK 래퍼 (Qwen 등 호환) |
| **file_parser** | `backend/app/utils/file_parser.py` | PDF/텍스트 파싱 (charset 감지) |

### 주요 함수

```python
from app.services.simulation_config_generator import generate_simulation_config
from app.services.report_agent import ReportAgent
from app.utils.llm_client import LLMClient

# 시뮬레이션 설정 생성
config = generate_simulation_config(
    seed_text="시드 텍스트 (뉴스, 정책 초안 등)",
    prediction_requirement="예측 요구사항"
)

# ReportAgent 초기화 및 리포트 생성
agent = ReportAgent(llm_client=LLMClient())
...
1. **도구 정의**: `backend/app/services/report_agent.py`에 도구 스키마 추가
2. **도구 구현**: 도구 실행 로직 구현
3. **테스트**: 단위 테스트 추가
