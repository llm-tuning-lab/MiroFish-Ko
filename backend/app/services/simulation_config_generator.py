...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        """
        logger.info(
            f"시작생성시뮬레이션설정: simulation_id={simulation_id}, 엔터티={len(entities)}"
        )
        ...
        for batch_idx in range(num_batches):
            ...
            report_progress(
                3 + batch_idx, f"생성Agent설정 ({start_idx + 1}-{end_idx}/{len(entities)})..."
            )
...
- reasoning에는 설정 이유를 한국어 한 문장으로 작성하세요."""
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        if agents_per_hour_min > num_entities:
            logger.warning(
                f"agents_per_hour_min({agents_per_hour_min})가 엔터티 수({num_entities})를 초과해 보정합니다."
            )
            agents_per_hour_min = max(1, num_entities // 10)

        if agents_per_hour_max > num_entities:
            logger.warning(
                f"agents_per_hour_max({agents_per_hour_max})가 엔터티 수({num_entities})를 초과해 보정합니다."
            )
        ...
        if agents_per_hour_min >= agents_per_hour_max:
            ...
            logger.warning(
                f"agents_per_hour_min이 max 이상이라 보정했습니다: {agents_per_hour_min}"
            )
    ...
    def _assign_initial_post_agents(
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                logger.warning(
                    f"poster_type '{poster_type}'에 맞는 에이전트가 없어 대체 매핑합니다."
                )
                if agent_configs:
                    # 영향력 높은 에이전트 우선
            ...
            logger.info(
                f"초기 게시물 매핑: poster_type='{poster_type}' -> agent_id={matched_agent_id}"
            )
